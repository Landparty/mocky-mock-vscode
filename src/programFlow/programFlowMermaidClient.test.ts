...
describe('fetchProgramFlowMermaid', () => {
  ...
  it('passes --format mermaid through, never --copybook-path (line numbers must match the unexpanded source)', async () => {
    let capturedArgs: string[] = [];
    const run: CommandRunner = async (_exe, args) => {
      capturedArgs = args;
      return { code: 0, stdout: 'flowchart TD\n', stderr: '' };
    };
    await fetchProgramFlowMermaid(run, 'mockymock', '/x/TEST.cbl');
    assert.deepStrictEqual(capturedArgs, ['analyze', 'program-flow', '/x/TEST.cbl', '--format', 'mermaid']);
  });
});
